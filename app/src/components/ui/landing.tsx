...
import { createPersonDocument, getPersonFromFirestore, updatePersonDoc } from "@/utils/firebase";
...
export const Landing = ({
  ...
  const [peopleScanResult, setPeopleScanResult] = useState(null);
  ...
  useEffect(() => {
    ...
    if (docId) {
      ...
      const intervalId = setInterval(() => {
        getPersonFromFirestore(docId).then(data => {
  ...
  const sendImageToAPI = async (imageDataUrl) => {
    ...
    let numberOfScans = localStorage.getItem('numberOfScans');
    if (!numberOfScans) {
      localStorage.setItem('numberOfScans', '1');
    } else {
      numberOfScans = parseInt(numberOfScans);
      if (numberOfScans >= 2) {
    ...
    try {
      const response = await fetch('/api/facecheck', {
        ...
        body: JSON.stringify({ imageData: imageDataUrl, personDocID: personDocID}),
      });
      const data = await response.json();

      if (response.status === 200) {
        console.log('success!!')
        window.location.href = '/?docId=' + personDocID;
      } else {
        console.log('error??')
  ...
  return (
    ...
    <div className="flex flex-col justify-center space-y-4">
      <div className="space-y-2">
        <h1 className="text-lg md:text-7xl font-normal pb-4 text-center bg-clip-text text-transparent bg-gradient-to-b from-neutral-100 to-neutral-300">Whomane</h1>
        <p className="text-xs md:text-xl font-normal text-center text-neutral-400 mt-4 max-w-lg mx-auto">An open source wearable with camera.</p>
      </div>
    ...
    <p className="text-neutral-400 font-bold my-4">Try it for yourself ⤵</p>
        ...
        <div className="aspect-[1/1] overflow-hidden rounded-lg">
            ...
            <>
              <div style={{ position: 'relative', height: '100%', }}>
                ...
                <div style={{ position: 'absolute', bottom: '2%', left: '50%', transform: 'translate(-50%, -50%)' }}>
                  <ShimmerButton shimmerColor={"#000000"} onClick={takePhoto} disabled={loading}>{loading?'Processing...':'Take photo'}</ShimmerButton>
                </div>
              </div>
              {image && (
                <Image
    ...
    <main className="flex flex-col bg-black">
        {peopleScanResult && <PersonCard key={peopleScanResult?.id} {...peopleScanResult} updatePersonDoc= {updatePersonDoc}/>}
      </main>
