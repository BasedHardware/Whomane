...
import { addSocialsToPersonDocument } from '@/utils/firebase';
...
const search_by_face = async (imageUrl: string): Promise<[string | null, any[] | null]> => {
    ...
    const responseImage = await axios.get(await url.text(), { responseType: 'arraybuffer' });
    ...
    const { data } = await axios.post<UploadResponse>(site + '/api/upload_pic', form, {
        headers: {
            ...form.getHeaders(),
            'accept': 'application/json',
            'Authorization': APITOKEN
    ...
    const id_search = data.id_search;

    console.log(`${data.message} id_search=${id_search}`);
    ...
    while (true) {
        ...
        const { data } = await axios.post(site + '/api/search', json_data, { headers: headers });
